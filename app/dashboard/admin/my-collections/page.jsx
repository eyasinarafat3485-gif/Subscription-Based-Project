...
export default function AdminMyCollectionsPage() {
  ...
  const handleDownload = (item) => {
    if (item.downloadUrl) {
      toast.info(`Starting download for ${item.title || item.productTitle}...`);
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="py-20 text-center space-y-3 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-xs font-bold text-slate-500">Loading your collections archive...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="py-16 text-center space-y-3 bg-white rounded-2xl border border-slate-200 p-8 shadow-xs">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Items downloaded from your admin account will be logged here.
          </p>
        </div>
      ) : (
